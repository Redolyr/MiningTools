	...
	public BlockIO(FMLPreInitializationEvent event) {
		if (Proxy.proxy.Config().BlocksIO == true) {
			...
			try {
				...
				GameRegistBlock1.setComment("Super Glow Stone GameRegist");

				if (Proxy.proxy.Config().GameRegistBlock1 = GameRegistBlock1.getBoolean(Proxy.proxy.Config().GameRegistBlock1)) {
					Proxy.proxy.Blocks().SuperGlowStone = new BlockSuperGlowStone(Material.ROCK, Proxy.proxy.getMiningToolsLocation(), Proxy.proxy.String().Name_Tag_56300, 1.5F, 1.0F, 100.0F, 0);
				}
